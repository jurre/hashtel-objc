...
- (HASColor *)colorForString:(NSString *)string {
  ...
  NSInteger originalGreen =
      [self hashCodeForString:[self reverseString:string]] & colorOffset;
...
- (NSInteger)hashCodeForString:(NSString *)string {
  NSMutableArray *stringBuffer =
      [NSMutableArray arrayWithCapacity:[string length]];
  for (NSInteger i = 0; i < [string length]; i++) {
    [stringBuffer
        addObject:[NSString
                      stringWithFormat:@"%C", [string characterAtIndex:i]]];
  }
